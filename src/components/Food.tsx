...
import textBackground from "../../images/purpelBg.svg";
    ...
    const fetchTopStories = async () => {
      try {
        const response = await axios.get(
          "https://api.nytimes.com/svc/topstories/v2/food.json?api-key=Zh4TZgpd9gf0KJcHM7Xemue3RU4ytS7E"
        );
      ...
      {articles.slice(0, 1).map((article) => (
        <div className="flex gap-10  " key={article.uri}>
          <div
            ...
            className="w-48 h-44"
          >
            <p className="font-medium text-[100px] text-center">F</p>
          </div>
